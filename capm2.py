...
# def close_all_positions_and_check(session):
#     positions, _ = new_get_positions_and_prices(session)
...
#             })
#             print(f"Adjusting {ticker} position by {action}ing {quantity} shares.")

#     # Wait a short period for orders to execute
#     sleep(1)  # Adjust based on your system's latency

#     # Re-check positions to confirm all are closed
#     updated_positions, _ = new_get_positions_and_prices(session)
#     all_closed = all(position == 0 for position in updated_positions.values())

#     if all_closed:
#         print("All positions are closed.")
#         # Break out of the loop or end execution here
#         return True
#     else:
#         print("Some positions are still open.")
#         return False
...
#def buy_or_sell(session, adjusted_return):
    ...
    for ticker, exp_return in expected_return.items():
        if float(exp_return)*100 > float(expected_market_return)*100 and float(expected_market_return)*100 > 0 and float(exp_return)*100 > float(risk_free_rate)*100:
            session.post('http://localhost:9999/v1/orders', params={'ticker': ticker, 'type': 'MARKET', 'quantity': 5000, 'action': 'BUY'})
            print(f"buy to open")
        ...
        elif float(exp_return)*100 < float(expected_market_return)*100 and float(expected_market_return)*100 < 0 and float(exp_return)*100 < float(risk_free_rate)*100:
            session.post('http://localhost:9999/v1/orders', params={'ticker': ticker, 'type': 'MARKET', 'quantity': 5000, 'action': 'SELL'})
...
def main():
    with requests.Session() as session:
        ...
        while get_tick(session) < 599 and not shutdown:
            ...
            # if keyboard.is_pressed('k'):  # Check if 'k' is pressed
            #     print('Closing positions...')
            #     close_position(session)
            #     close_all_positions_and_check(session)
