...
#def buy_or_sell(session, adjusted_return):
    ...
    for exp_return in expected_return.items():
        if float(exp_return)*100 > float(expected_market_return)*100 and float(expected_market_return)*100 > 0.25:
            session.post('http://localhost:9999/v1/orders', params={'ticker': 'ALPHA', 'type': 'MARKET', 'quantity': 5000, 'action': 'BUY'})
            print(f"buy to open")
            print(f"bought...{ticker}")
            #print(f"exp return: {exp_return*100} > percent diff: {percentage_diff+0.1} and > {risk_free_rate*100+0.1}")

        elif float(exp_return)*100 < float(expected_market_return)*100 and float(expected_market_return)*100 < -0.25:
            session.post('http://localhost:9999/v1/orders', params={'ticker': 'ALPHA', 'type': 'MARKET', 'quantity': 5000, 'action': 'SELL'})
