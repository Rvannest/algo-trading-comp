...
SPEEDBUMP = 0.1
MAX_VOLUME = {'HAWK': 5000, 'DOVE': 5000, 'RIT_C': 5000, 'RIT_U': 5000}
...
profits = {'HAWK': 0, 'DOVE': 0, 'RIT_C': 0, 'RIT_U': 0}
...
def main():
    single_side_transaction_time = {'HAWK': 0, 'DOVE': 0, 'RIT_C': 0, 'RIT_U': 0}
    # dictionary that tracks whether a single side of MM transaction has been filled for each symbol
    single_side_filled = {'HAWK': False, 'DOVE': False, 'RIT_C': False, 'RIT_U': False}
    ...
    with requests.Session() as s:
        ...
        while tick > 0 and tick < 295 and not shutdown:
            for sym in ['HAWK', 'DOVE', 'RIT_C', 'RIT_U']:
                ...
                # else, when there is an open order for the current security
                    ...
                    if (open_sells_volume == 0):
                        ...
                        # enter elif if NO continue. If at least 3 ticks have passed since a single side of an order was filled
                            ...
                            if(potential_profit >= .01 or tick - single_side_transaction_time[sym] >= 2):
                                #buy at the current bid
                                action = 'BUY'
